...
class ReportCommentIcon extends StatelessWidget {
  final CommentModel commentModel;

  const ReportCommentIcon({required this.commentModel});
...
class FavoriteIconContent extends StatefulWidget {
  ...
  const FavoriteIconContent({super.key, required this.viewModel, required this.commentModel});
...
class _FavoriteIconContentState extends State<FavoriteIconContent> {
  ...
  void didUpdateWidget(covariant FavoriteIconContent oldWidget) {
    ...
    Future.delayed(Duration.zero, () {
      if (this.widget.viewModel.reportCommentReport.status == ActionStatus.running) {
        if (progressDialog == null) progressDialog = new ProgressDialog(context);
        if (!progressDialog!.isShowing()) {
          progressDialog!.setMessage("");
          progressDialog!.show();
        }
      } else if (this.widget.viewModel.reportCommentReport.status == ActionStatus.error) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }
        showToast("${this.widget.viewModel.reportCommentReport.msg}");
        this.widget.viewModel.reportCommentReport.status = null;
      } else if (this.widget.viewModel.reportCommentReport.status == ActionStatus.complete) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        this.widget.viewModel.reportComment(widget.commentModel.id!);
      },
      child: Image.asset(
