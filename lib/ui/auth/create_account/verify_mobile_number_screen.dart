...
class VerifyMobileNumberView extends StatelessWidget {
  ...
  const VerifyMobileNumberView({super.key, required this.phone});
...
class _VerifyMobileNumberViewState extends State<_VerifyMobileNumberViewContent> {
  ...
  void didUpdateWidget(covariant _VerifyMobileNumberViewContent oldWidget) {
    ...
    Future.delayed(Duration.zero, () {
      if (this.widget.authViewModel.verifyActionReport.status == ActionStatus.running) {
      ...
      } else if (this.widget.authViewModel.verifyActionReport.status == ActionStatus.error) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }
        showToast("${this.widget.authViewModel.verifyActionReport.msg}");
        this.widget.authViewModel.verifyActionReport.status = null;
      } else if (this.widget.authViewModel.verifyActionReport.status == ActionStatus.complete) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Center(
                    child: Text(
                      '${Translations.of(context).text("verificationScreenSubTitle")} +965${this.widget.phone}',
                      style: TextStyle(
                  ...
                  Padding(
                    ...
                    child: PinCodeFields(
                      onCompleted: (newOTP) {
                        this.widget.authViewModel.verify(
                              this.widget.phone,
                              newOTP,
                            );
                      },
                  ...
                  timerFinished
                      ? InkWell(
                          onTap: () {
                            ...
                            this.widget.authViewModel.resendOTP("965${widget.phone}");
                          },
                          child: Text(
