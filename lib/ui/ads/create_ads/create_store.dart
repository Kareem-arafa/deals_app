...
class _CreateStoreContent extends StatefulWidget {
  ...
  const _CreateStoreContent({required this.viewModel});
...
class _CreateStoreState extends State<_CreateStoreContent> {
  ...
  void didUpdateWidget(covariant _CreateStoreContent oldWidget) {
    ...
    Future.delayed(Duration.zero, () {
      if (this.widget.viewModel.createStoreReport.status == ActionStatus.running) {
        if (progressDialog == null) progressDialog = new ProgressDialog(context);
        if (!progressDialog!.isShowing()) {
          progressDialog!.setMessage("");
          progressDialog!.show();
        }
      } else if (this.widget.viewModel.createStoreReport.status == ActionStatus.error) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }

        showToast("${this.widget.viewModel.createStoreReport.msg}");
        this.widget.viewModel.createStoreReport.status = null;
      } else if (this.widget.viewModel.createStoreReport.status == ActionStatus.complete) {
        if (progressDialog != null && progressDialog!.isShowing()) {
          progressDialog!.hide();
          progressDialog = null;
        }
    ...
    return Column(
      ...
      children: [
        ...
        ButtonWidget(
          ...
          title: Translations.of(context).text("createStoreCreateStoreButtonTitle"),
          onTap: () {
            if (!isChecked) {
